...
brightness_slider_value_changed_cb (CcBrightnessScale *self, GtkRange *range)
{
  ...
  if (self->device == BRIGHTNESS_DEVICE_KBD)
    variant = g_variant_new_parsed ("('io.github.gnome_de.SettingsDaemon.Power.Keyboard',"
                                    "'Brightness', %v)",
                                    g_variant_new_int32 (percentage));
  else
    variant = g_variant_new_parsed ("('io.github.gnome_de.SettingsDaemon.Power.Screen',"
                                    "'Brightness', %v)",
                                    g_variant_new_int32 (percentage));
...
cc_brightness_scale_constructed (GObject *object)
{
  ...
  if (self->device == BRIGHTNESS_DEVICE_KBD)
    interface = "io.github.gnome_de.SettingsDaemon.Power.Keyboard";
  else
    interface = "io.github.gnome_de.SettingsDaemon.Power.Screen";

  cc_object_storage_create_dbus_proxy (G_BUS_TYPE_SESSION,
                                       G_DBUS_PROXY_FLAGS_NONE,
                                       "io.github.gnome_de.SettingsDaemon.Power",
                                       "/io/github/gnome_de/SettingsDaemon/Power",
                                       interface,
                                       self->cancellable,
                                       got_proxy_cb,
