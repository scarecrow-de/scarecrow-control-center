...
brightness_slider_value_changed_cb (CcBrightnessScale *self, GtkRange *range)
{
  ...
  if (self->device == BRIGHTNESS_DEVICE_KBD)
    variant = g_variant_new_parsed ("('io.github.scarecrow_de.SettingsDaemon.Power.Keyboard',"
                                    "'Brightness', %v)",
                                    g_variant_new_int32 (percentage));
  else
    variant = g_variant_new_parsed ("('io.github.scarecrow_de.SettingsDaemon.Power.Screen',"
                                    "'Brightness', %v)",
                                    g_variant_new_int32 (percentage));
...
cc_brightness_scale_constructed (GObject *object)
{
  ...
  if (self->device == BRIGHTNESS_DEVICE_KBD)
    interface = "io.github.scarecrow_de.SettingsDaemon.Power.Keyboard";
  else
    interface = "io.github.scarecrow_de.SettingsDaemon.Power.Screen";
