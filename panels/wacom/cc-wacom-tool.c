			     ...
			     GError       **error)
{
	...
	if (tool->serial == 0) {
		...
		path = g_strdup_printf ("/io/github/gnome_de/desktop/peripherals/stylus/default-%s:%s/",
					vendor, product);
        } else {
		path = g_strdup_printf ("/io/github/gnome_de/desktop/peripherals/stylus/%lx/", tool->serial);
        }

	tool->settings = g_settings_new_with_path ("io.github.gnome_de.desktop.peripherals.tablet.stylus",
						   path);
	g_free (path);
