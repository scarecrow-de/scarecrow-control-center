...
gsd_device_get_settings (GsdDevice *device)
{
	...
	if (type & (GSD_DEVICE_TYPE_TOUCHSCREEN | GSD_DEVICE_TYPE_TABLET)) {
		...
		if (type & GSD_DEVICE_TYPE_TOUCHSCREEN) {
			schema = "io.github.gnome_de.desktop.peripherals.touchscreen";
			path = g_strdup_printf ("/io/github/gnome_de/desktop/peripherals/touchscreens/%s:%s/",
						vendor, product);
		} else if (type & GSD_DEVICE_TYPE_TABLET) {
			schema = "io.github.gnome_de.desktop.peripherals.tablet";
			path = g_strdup_printf ("/io/github/gnome_de/desktop/peripherals/tablets/%s:%s/",
						vendor, product);
		}
	} else if (type & (GSD_DEVICE_TYPE_MOUSE | GSD_DEVICE_TYPE_TOUCHPAD)) {
		schema = "io.github.gnome_de.desktop.peripherals.mouse";
	} else if (type & GSD_DEVICE_TYPE_KEYBOARD) {
		schema = "io.github.gnome_de.desktop.peripherals.keyboard";
	} else {
		return NULL;
	}
